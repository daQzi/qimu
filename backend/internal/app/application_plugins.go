...
func (s *Service) applicationPlugins() *plugins.Service {
	...
	return plugins.New(s.repo, s.dataDir, setting == "" || setting == "true" || setting == "1").WithAdapters(pluginHostAdapters()).WithRemoteHost(pluginRemoteHost{svc: s}).WithModelHost(pluginModelHost{svc: s}).WithInputValidator(validatePluginMediaInput).WithRunAccess(func(repo *repository.Repository, user string) error {

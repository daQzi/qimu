...
func TestP08MigrationPreservesPluginState(t *testing.T) {
	for _, driver := range []string{"sqlite", "postgres"} {
		t.Run(driver, func(t *testing.T) {
			db := p02LineageDatabase(t, driver, "p08")
			...
			for _, table := range []any{&model.AgentThreadEntry{}, &model.AgentThreadCanvas{}, &model.AgentThread{}} {
			...
			if !db.Migrator().HasIndex(&model.AgentThreadEntry{}, "idx_agent_thread_entry_key") {

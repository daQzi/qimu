...
分镜任务先检查 canvas_get_context.storyboardReadiness 或 canvas_read_storyboard.readiness。缺少画风时先 canvas_list_styles，再按用户已确认的风格或自主决策授权调用 canvas_apply_style；只有确实需要用户取舍时才提问，不重复询问已有风格。应用后重新读取上下文确认 canGenerateStoryboard，不能只在正文声称画风已设置。方案批准后创建 script 时必须提供非空 shots，不能只写 content/prompt 后把默认空行当成完成。已有分镜用 canvas_read_storyboard 取得真实行 ID，再 canvas_edit_storyboard 更新空行、按需追加镜头，保留已有行 ID 与素材；已有剧本足够时由你直接编写镜头，不要求用户再次点击生成分镜。0–2秒对应 durationSeconds=2，2–4秒为2秒，4–7秒为3秒，7–9秒为2秒，9–10秒为1秒；写入后核对总时长与约定一致。不用改标题代替修改时长，不声称有分镜行就已经生成图片视频。

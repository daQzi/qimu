...
export function RequireFeature({ feature, children }: { feature: FeatureKey; children: ReactNode }) {
    ...
    if (error) return <WorkspacePage><WorkspaceErrorState title="无法确认功能状态" description={error} actionLabel="返回创作台" onRetry={() => navigate("/home", { replace: true })} /></WorkspacePage>;
    if (!adminBypass && !features[feature]) {
        ...
        const backPath = isAdminFeature ? "/admin" : "/home";

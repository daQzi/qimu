...
export function PluginCanvasSave({ run, canvasId, action, onCreated }: { run: PluginRunView; canvasId: string; action: PluginCanvasAction; onCreated: (id: string) => void }) {
    ...
    const save = async (newInstance: boolean) => {
        ...
        try {
            ...
            if (!attempt.current || newInstance) {
                ...
                if (useUserStore.getState().user?.id !== userID) return;
                attempt.current = { key: crypto.randomUUID(), request: { operation: action.operation, releaseId: action.releaseId, context: { hostSurface: "canvas", canvasId }, input: { runId: run.id, resultDigest: run.resultRef.digest, blueprintId: action.blueprintId, snapshotHash: snapshot.snapshotHash, instanceKey: newInstance ? crypto.randomUUID() : "default" } } };
            }
    ...
    return <div className="space-y-2">
        <div className="flex flex-wrap gap-2">
            <Button size="small" loading={busy} onClick={() => void save(false)}>保存到当前画布</Button>
            <Button size="small" disabled={busy} onClick={() => void save(true)}>另建结果节点</Button>
            {error && <Button size="small" disabled={busy} onClick={() => { attempt.current = undefined; setError(""); }}>重新读取画布后再试</Button>}
        </div>

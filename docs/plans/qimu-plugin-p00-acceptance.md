...
> 阶段状态：已由用户验收通过，提交 `0fe8d904`。下文保留 P00 交付时的范围记录；P01 对安装与技能目录的扩展以其阶段验收说明为准。
> 起始版本：`9eb2fcd4`（v1.5.1）。关联[统一实施手册](qimu-agent-plugin-platform-execution-runbook.md)。

后续分支策略变更：用户要求此次平台改造先保留在功能分支，main 中 P00 使用反向提交撤回；P00 内容仍保存在 `codex/plugin-p01-installation`，下文的首次合并记录仅用于追溯。
